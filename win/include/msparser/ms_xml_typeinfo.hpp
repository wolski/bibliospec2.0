...
#                                                                            #
##############################################################################
 * @(#)$Source: /export/cvsroot/parser/inc/ms_xml_typeinfo.hpp,v $
 * @(#)$Revision: 1.4 $
 * @(#)$Date: 2007-01-31 18:25:26 $
##############################################################################
 */
...
namespace matrix_science {
    ...
    //! An abstract interface for classes that represent complex elements in .xml-file
    /*!
     *  For languages that support polimorphism and virtual inheritance (like C++ and Java) this interface can be used 
     * for uniform validation of all xml-objects. Every xml-class is derived from this interface
     * and, therefore, an instance of such classes cal be passed to validation-methods of ms_xml_schema-class.
     * Alternatively, values of simple types can be validated with validateSimpleXXX()-method of 
     * ms_xml_schema-class and complex objects can be validated using their own validation methods.
     */
     ...
     * $x = $msparser::MS_UMOD_ATTRIBUTE_USE
     * </tt></P>
     */
    ...
    //! This class is used to represent a type for elements from .xml-file
    /*!
     * Used in connection with ms_xml_schema-class.
     */
    ...
    public:
        ...
        //! Does a restriction have maxInclusive property for integers and doubles?
        bool hasMaxInclusive() const;

        //! Returns double maxInclusive property value that can be cast to an integer if needed
        double getMaxInclusive() const;

        //! Does a restriction have minLength property for strings?
        bool hasMinLength() const;

        //! Returns integer minLength property value
        int getMinLength() const;

        //! Does a restriction have maxLength property for strings?
        bool hasMaxLength() const;

        //! Returns integer maxLength property value
