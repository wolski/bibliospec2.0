...
namespace matrix_science {
    ...
    //! The class represents a single taxonomy choice entry in "taxonomy" file
    ...
    public:
        ...
        //! Allows to change the title for taxonomy choice
        void setTitle(const char* value);

        //! Returns a total number of included taxonomy IDs
        int getNumberOfIncludeTaxonomies() const;

        //! Can be used to traverse included taxonomy IDs
        int getIncludeTaxonomy(const int n) const;

        //! Deletes all taxonomy IDs from include-list
        void clearIncludeTaxonomies();

        //! Adds one more taxonomy ID into the include-list
        ...
        //! Can be used to traverse excluded taxonomy IDs
        int getExcludeTaxonomy(const int n) const;

        //! Deletes all taxonomy IDs from exclude-list
        void clearExcludeTaxonomies();

        //! Adds one more taxonomy ID into the exclude-list
    ...
    //! Use this class in order to read in taxonomy file
    /*!
     *  The list of taxonomy choices in the search form is taken from the taxonomy file. The file
     *  consists of several entries. The first line of each entry must start with the 
     *  <tt>Title:</tt> keyword, followed by a text string that is used to identify the 
     *  species in forms and reports. The definition should be short and self-explanatory. 
     *  To show the tree structure, indentation can be used. Unfortunately, it is not possible
     *  to use tabs or multiple spaces for indentation in an html form, so a full
     *  stop (period) and a space are used to indent the list. Internal spaces are significant, 
     *  and there should never be two or more spaces together. <BR>This should be followed with a 
     *  definition line starting with the <tt>Include:</tt> keyword, followed by one or more 
     *  numbers separated with commas. With the supplied MSDB database, and supplied taxonomy
     *  files, these numbers should be the NCBI taxonomy ID. <BR>This should be followed with a 
     *  definition line starting with the <tt>Exclude:</tt> keyword, followed by one or more 
     *  numbers separated with commas. Any sequence with a taxonomy ID that passes the 
     *  ‘include’ test, may then be rejected by any entry in the exclude list. Finally, 
     *  each entry must end with a <tt>*</tt>.
     */
